...
const API_URL = "https://eth-mainnet.alchemyapi.io/v2/qmuA1_caZxbrsZi3TO5TH2isPwtjxon2"; //alchemyapi
const web3 = createAlchemyWeb3(API_URL);

const contractABI = require("./abi.json");
const contractAddress = "0x13Bf84c626E0e3cD92aED83fBF288ACB4f3ec8d8";  //smart contract address
...
//     console.log("valid for whitelist - inside function checkIfValidWl", isValid, proof);
...
//     await nftContract.methods.whitelistMint(amount, proof)
//     .send({
//         from: account,
//         to: contractAddress,
...
//         response.status = "✅ Successfully Minted "+amount+" Super Dudes";
...
//     await nftContract.methods.mint(amount)
//     .send({
//         from: account,
//         to: contractAddress,
//         value: String(costWEI * amount)
...
//     await nftContract.methods.claimFromParentNFT(amount)
//     .send({
//         from: account,
//         to: contractAddress
//     })
...
export const redeemToken = async (account) => {
    ...
    await nftContract.methods.redeem()
    .send({
        from: account,
        to: contractAddress
    })
    .then(function(receipt){
        console.log("receipt: ", receipt);
        response.success = true;
        response.status = "✅ Successfully Redeemed Super Dudes";
    }).catch(function(error){
        console.log("error: ", error);
        response.success = false;
        response.status = "😥" + error.message;
